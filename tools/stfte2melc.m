...
% (5) should use the [newish] 'zq' option of v_filtbankm to deal with the q.keepDC option
persistent q0
%
...
if strcmpi(q.groupdelay,'dct') % use DCT instead of DFT
    ...
    for it=1:nt                                                             % loop for all time frames
        ...
        elseif strcmpi(q.MELphase,'grpdel')                             % output group delay in seconds
        ...
        else                                                            % par.MELphase='linear' or'piecewiselin'
            %************************************************************************************************************
            % Phase cost function:                                                                                      *
            %                                                                                                           *
            ...
            if strcmpi(q.MELphase,'piecewiselin')
                ...
                mpsqit=sparse(1:nfftp,1:nfftp,sqrt(stftm2));            % diag matrix square root of phase error weights
                ...
                phden=phlinw'*phlinw+rega*eye(nmel);                    % denominator matrix
                ...
                while ~numel(ssqold) || ssq<ssqold*thresh && loopcount<loopmax      % loop until the error no longer reduces (should perhaps be the weighted error here)
                    ...
                    sdftait=mod(sdfta-sdftrait+pi,2*pi)+sdftrait-pi;    % phase of stft(freq,1) unwrapped to be in range sdftrait+-pi (+ve frequencies only)
