function [x,cf,xi,il,ih]=filtbankmd(p,n,fs,fl,fh,w)
% This is the same as v_filtbankm but with an additional option for DCT input
...
%
%             't' = input is DCT rather than DFT
%
...
ww=any(repmat('ncChHxXyYpPzZqdDuUsSgGwt',length(w),1)==repmat(w',1,24),1);    % decode all other options
% ww elements: 1=n,2=c,3=C,4=h,5=H,6=x,7=X,8=y,9=Y,10=p,11=P,12=z,13=Z,14=q,15=d,16=D,17=u,18=U,19=s,20=S,21=g,22=G,23=w, 24=t
...
elseif ww(24)                       % 't' option: n gives DCT length
    nf=n;                           % all inputs correspond to positive frequencies
    df=0.5*fs/n;                    % input frequency bin spacing
