% TIMIT phone recognition with Gaussian mixtures + group delay features
%
% Ideas:
...
%
parvar={'cfname' 'nmix'};                                                              % cell array row listing the parameters to change during trials
configpars={  % one row per trial giving the parameter values: the parameters listed in parvar come first followed by parameter-value pairs
    {'30ms-g1' 1  'epoch' 0 'fixfl' 0.03 'ovfact' 2 'window' 'n'};
    % {'30ms-m4' 0 4};
    {'ep-g1' 1};
       % {'30ms-m4' 0 4};         
    {'ep-fmg-g1' 1 'feats' 'mg'}; 
    {'ep-fmg-l15-g1' 1 'feats' 'mg' 'ldatype' 'lda'}; 
...
par0.nogsf=       0;                 % 0= use stfte, 1= use old-fashioned processing
...
par0.feats=       'm';              % 'M' = Mel features per-grid, post-grid features: 'm'= Mel, 'r'=real, 'i'=imag, 'g'=gdshew, 'd'=delta-gd.
% = gs_frames =
...
% = stftegrid =
par0.interpstft=  'indep';            % interpolation method for call to griddata: {'none','indep','nearest','linear','natural','cubic','v4'}
par0.interpflen=  0.03;             % interpolation target frame length in seconds (reciprocal of frequency-grid spacing in Hz) [0.01]
...
featch='Mmrigd';                            % feature vector components in par0.feats
nfeatch=length(featch);
%
% derived parameters
%
...
for icfg=1:ncfg                                         % loop through parameter configurations
    ...
    %
    needlda=~strcmp(par.ldatype,'none'); % true if we need to do LDA transformation on the features
    feats=par.feats; % selected features
    nfeats=length(feats);
    featmap=repmat(feats',1,nfeatch)==repmat(featch,nfeats,1);
    ndp=par.ndp; % number of alternatives to keep in the dynamic programming
    ndm=(par.ncep+any(par.wcep=='0'))*(1+any(par.wcep=='d')+any(par.wcep=='D'));    % size of mel feature vector
    featlens=[ndm ndm repmat(1+floor(round(par.interpflen*fs)/2),1,4)]; % length of each feature vector component
    featdivs=cumsum(sum(featmap.*repmat(featlens,nfeats,1),2))'+0.5; % feature dividing lines
    nd=sum(featmap,1)*featlens';
    ...
    nfft=round(par.fixfl*fs);                           % (for fixed frames only) frame length in samples [~ par.fixfl seconds]
    inc=floor(nfft/2);                                  % (for fixed frames only) frame increment = 50% of frame length
    for itt=1:2                                         % 1=training and 2=testing
        ...
        for ifile=1:nfiles                                          % loop through training or testing files
            ...
            % if ~par.nogsf                                           % *** epoch-based frames ***
            if par.epoch
                ...
                nfft=round(par.fixfl*fs);                       % (for fixed frames only) frame length in samples [~ par.fixfl seconds]
                inc=floor(nfft/par.ovfact);                     % (for fixed frames only) frame increment = 50% of frame length
                nframe=floor((length(s)-nfft)/inc-1);
                metain=[1+(0:nframe-1)'*inc repmat(nfft,nframe,1)]; % one row per frame containing [start-sample length]
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % Calculate the feature vectors %
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            [stft,meta,gdsh,grpd,dgrpd]=stfte(s,metain,[],par);                 % epoch-based STFT
            if any(feats=='M')
                [melbm,melbu,cfhz,mbm]=stfte2melc(stft,fs,par.nmel,par);
                ce=mel2melcep(melbm,par.wcep,par.ncep);              % calculate the mel-cepstrum feature vector, ce
            end
            gdshew=v_modsym(gdsh-repmat(meta(:,8),1,size(gdsh,2)),meta(:,3));      % shifted group delay with EWPD subtracted
            grid=[meta(:,1:2) repmat(round(par.interpflen*fs),size(meta,1),1)];
            interpnfftp=1+floor(grid(1,3)/2); % number of positive frequencies in grid-based parameters
            [stftg,meta]=stftegridz(cat(3,stft,gdshew,dgrpd),meta,grid,par,[-2 3 4]);          % map onto a fixed grid in frequency only
            stft=stftg(:,:,1); % grid-based stft
            gd=stftg(:,:,2); % grid-based group delay
            hd=stftg(:,:,3); % grid-based group delay derivative
            [melbm,melbu,cfhz,mbm]=stfte2melc(stft,fs,par.nmel,par); % grid-based met cepstrum
            cg=mel2melcep(melbm,par.wcep,par.ncep);              % calculate the mel-cepstrum feature vector, c
            ...
            y=log(max(melbm,pth));                              % mel log-spectrogram (for plotting only)
            c=zeros(size(stft,1),0); % initialize feature vector array
            for i=1:length(feats) % %%% this could be made faster using indexing and featmap
                switch feats(i)
                    case 'M'
                        c=[c ce];
                    case 'm'
                        c=[c cg];
                    case 'r'
                        c=[c real(stft(:,1:interpnfftp))];
                    case 'i'
                        c=[c imag(stft(:,1:interpnfftp))];
                    case 'g'
                        c=[c gd(:,1:interpnfftp)];
                    case 'd'
                        c=[c hd(:,1:interpnfftp)];
                end
            end
            % else                                                    % *** fixed frames ***
    ...
    if any(icfg==parplt)                                            % need to do plots for this configuration
        ...
        if any(pltsel==600) %  600: Cepstrogram + recognition results
            ...
            axis xy;
            if ~needlda && length(featdivs)>1
                hold on;
                plot([1; nframe],[1; 1]*featdivs(1:end-1),'-w');
                hold off;
            end
